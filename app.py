...
def parse_mrz(text: str) -> dict:
    """
    Parsing MRZ (Machine Readable Zone) paspor.
    Format MRZ paspor: 2 baris, masing-masing 44 karakter.
    """
    lines = [l.strip() for l in text.split('\n') if l.strip()]
    mrz_lines = [l for l in lines if re.match(r'^[A-Z0-9<]{40,}$', l.replace(' ', ''))]

    result = {}
    if len(mrz_lines) >= 2:
        line1 = mrz_lines[0].replace(' ', '').ljust(44, '<')[:44]
        line2 = mrz_lines[1].replace(' ', '').ljust(44, '<')[:44]

        # Baris 1: nama (posisi 5-44)
        name_field = line1[5:44]
        parts = name_field.split('<<')
        if parts:
            surname = parts[0].replace('<', ' ').strip()
            given  = parts[1].replace('<', ' ').strip() if len(parts) > 1 else ''
            result['nama'] = f"{surname} {given}".strip() if given else surname

        # Baris 2: nomor paspor, kewarganegaraan, DOB, expiry
        ...
        exp = line2[19:25]   # YYMMDD
        if re.match(r'\d{6}', exp):
            yy_exp = int(exp[:2])
            year_exp = 2000 + yy_exp
            result['masa_berlaku'] = f"{exp[4:6]}/{exp[2:4]}/{year_exp}"

    return result

def extract_from_text(text: str) -> dict:
    """Ekstraksi dari teks bebas (fallback jika MRZ tidak terdeteksi)."""
    result = {}
    lines = [l.strip() for l in text.split('\n') if l.strip()]
    full_text = ' '.join(lines)

    # Nomor Paspor (A/B/C/E/P diikuti 6-8 digit)
    m = re.search(r'\b([A-Z]{1,2}\d{6,8})\b', full_text)
    if m:
        result['nomor_paspor'] = m.group(1)

    # Tanggal — format DD MMM YYYY atau DD/MM/YYYY
    date_patterns = [
    ...
    for pat in date_patterns:
        dates_found += re.findall(pat, full_text, re.IGNORECASE)
    ...
    # Kewarganegaraan (3-huruf kode negara)
    m = re.search(r'\b(IDN|MYS|SGP|PHL|THA|VNM|CHN|IND|PAK|BGD|LKA|NPL|MMR|KHM|LAO|BRN|TLS|USA|GBR|AUS|NZL|CAN|DEU|FRA|NLD|BEL|CHE|AUT|ITA|ESP|PRT|SWE|NOR|DNK|FIN|POL|CZE|HUN|ROU|BGR|HRV|SRB|BIH|MKD|ALB|GRC|TUR|RUS|UKR|KAZ|UZB|TKM|TJK|KGZ|AZE|ARM|GEO|ISR|SAU|ARE|QAT|KWT|BHR|OMN|YEM|IRQ|IRN|JOR|LBN|SYR|EGY|LBY|TUN|DZA|MAR|ZAF|NGA|GHA|KEN|ETH|TZA|UGA|RWA|BDI|MOZ|ZMB|ZWE|BWA|NAM|MWI|SEN|CIV|CMR|COD|AGO|MDG|SOM|SDN|SSD|ERI|DJI|COM|MUS|SYC|CPV|STP|GNQ|GAB|COG|CAF|TCD|NER|MLI|BFA|GIN|SLE|LBR|GMB|GNB|MRT|TGO|BEN|HTI|JM|TTD|BRB|LCA|VCT|GRD|DMA|ATG|KNA|BLZ|GTM|HND|SLV|NIC|CRI|PAN|MEX|CUB|DOM|PRI|BHS|ARG|CHL|URY|PRY|BOL|PER|ECU|COL|VEN|GUY|SUR|BRA)\b',
...
if st.session_state.step == 1:
    ...
    if uploaded:
        ...
        if st.button("🔍 Proses & Ekstrak Data Otomatis", key="btn_ocr"):
            with st.spinner("Memproses OCR... Mohon tunggu sebentar."):
