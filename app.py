...
def extract_from_text(text: str) -> dict:
    ...
    # ── 1. NOMOR PASPOR ──────────────────────────────────────
    # Paspor Indonesia: X diikuti 7 angka, contoh: X1098974
    ...
    m = re.search(
        r'(?:NAMA LENGKAP|FULL NAME)[^A-Z]*([A-Z][A-Z\s]{4,})',
        full, re.IGNORECASE
    )
    if m:
        nama_raw    = m.group(1).strip()
        nama_bersih = re.sub(r'\s+', ' ', nama_raw).strip()
        # Batasi maksimal 5 kata agar tidak ambil teks lain
        kata        = nama_bersih.split()[:5]
    ...
    m = re.search(
        r'(?:DATE OF EXPIRY|HABIS BERLAKU|TGL\.?\s*HABIS)[^0-9]*(\d{1,2}\s+[A-Z]{3}\s+\d{4})',
        full, re.IGNORECASE
    )
    if m:
        result['masa_berlaku'] = m.group(1).strip()
