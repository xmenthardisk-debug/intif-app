...
def parse_mrz(mrz_lines: list) -> dict:
    ...
    try:
        ...
        baris = []
        for l in mrz_lines:
            # Ganti karakter mirip < yang sering salah dibaca OCR
            cleaned = l.strip()
        ...
        # Nama dari baris 1
        # Format: P<IDNSURNAME<<GIVEN<NAMES<<<<
        if '<<' in line1:
            name_part = line1[5:] if line1.startswith('P<') else line1
            # Hapus kode negara di awal
        ...
        # Nomor paspor dari baris 2 (posisi 0-8)
        ...
        # Kewarganegaraan (cari IDN atau kode negara)
        neg = re.search(r'IDN|MYS|SGP|PHL|THA|CHN|IND|PAK|USA|GBR|AUS', line2)
        if neg:
            result['kewarganegaraan'] = neg.group(0)

        # Tanggal lahir dari baris 2
        # Cari pola YYMMDD setelah kode negara
        ...
        # Masa berlaku — cari M atau F diikuti 6 digit
...
def extract_from_text(text: str) -> dict:
    """Ekstrak data paspor Indonesia dari teks OCR."""
    result = {}
    lines  = [l.strip() for l in text.split('\n') if l.strip()]
    full   = ' '.join(lines)

    # ── 1. NOMOR PASPOR ──────────────────────────────────────
    # Hapus spasi di dalam teks dulu, lalu cari pola nomor paspor
    full_nospace = re.sub(r'\s+', '', full)
    m = re.search(r'([A-Z][0-9]{7,8})', full_nospace)
    if m:
        result['nomor_paspor'] = m.group(1)

    # ── 2. NAMA — ambil setelah "FULL NAME" ──────────────────
    # Cari nama yang muncul setelah label, skip kata-kata label itu sendiri
    m = re.search(
        r'(?:NAMA LENGKAP|FULL NAME)[\s/A-Z]{0,10}?\n([A-Z][A-Z\s]{4,})',
        full, re.IGNORECASE
    )
    if not m:
        # Fallback: cari baris yang berisi nama (baris kapital semua, bukan label)
        ...
        for line in lines:
            ...
            if (len(kata_kata) >= 2 and
                ...
                # Bersihkan kata pendek di depan (noise OCR seperti "PS", "oa")
    ...
    # ── 4. TANGGAL LAHIR — setelah "DATE OF BIRTH" ───────────
    m = re.search(
    ...
    # ── 5. MASA BERLAKU — setelah "DATE OF EXPIRY" ───────────
    # Cari SEMUA tanggal yang ada, lalu ambil yang paling akhir
    semua_tanggal = re.findall(
        r'(\d{1,2}\s+(?:JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)\s+\d{4})',
        full, re.IGNORECASE
    )
    if semua_tanggal:
        # Tanggal pertama = tanggal lahir, tanggal terakhir = masa berlaku
        ...
        if len(semua_tanggal) >= 2:
            ...
            # Coba cari juga tanggal pengeluaran
            result.setdefault('masa_berlaku', semua_tanggal[0])

    # ── 6. COBA MRZ SEBAGAI BACKUP ───────────────────────────
    mrz_lines = [
        l for l in lines
    ...
    if len(mrz_lines) >= 2:
        ...
        # Hanya pakai data MRZ jika data utama kosong
    ...
    if uploaded:
        ...
        if st.button("🔍 Proses & Ekstrak Data Otomatis", key="btn_ocr"):
            with st.spinner("Membaca data paspor... Mohon tunggu (5-10 detik)"):
                ...
                st.stop()  # ← tambahkan ini, jangan rerun dulu
    else:
